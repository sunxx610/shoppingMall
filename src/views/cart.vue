      ...
      <div class="cart-box">
        ...
        <div class="cart-inner">
          <div class="empty-label" v-if="totalCount<=0">
            ...
            <router-link class="link" to="/">Pick items now</router-link>
          </div>
          <div v-else>
            <div class="cart-table-title">
              <span class="name">Item information</span>
            ...
            <div class="cart-table">
              <div class="cart-group">
                <!--shopping list-->
                <div class="cart-top-items" v-for="item in cartListData">
                  <div class="cart-items">
                    <div class="items-choose">
                      <span
                          class="blue-checkbox-new"
                          :class="{'checkbox-on':item.checked}"
                          @click="editCart(item.sku_id,item.itemCount,item.checked)"
                    ...
                    <div class="operation">
                      <a
                          ...
                          @click="delCartActHandle(item.sku_id)"
                      ></a>
                    </div>
                    <div class="subtotal">{{(item.price*item.itemCount)|currency('$')}}</div>
                    <div class="item-cols-num">
                      <div class="select js-select-quantity">
                        <span
                            class="down"
                            :class="{'down-disabled':item.count<=1}"
                            @click="editCart(item.sku_id,item.itemCount,-1)"
                        >-</span>
                        <span class="num">{{item.itemCount}}
													</span>
                        <span
                            class="up"
                            @click="editCart(item.sku_id,item.itemCount,1)"
            ...
            <div>
              <div class="choose-all js-choose-all">
                <span
                    ...
                    @click="checkAllItemsActHandle(allChecked)"
                ><a href="javascript:;"></a></span>
                All
              </div>
              <div
                  class="delete-choose-goods"
                  @click="delAllCheckedItemsActHandle"
          ...
          <div class="shipping">
            <div class="shipping-box">
              <div class="shipping-total shipping-num">
                <h4 class="">
                  Selected <i>{{checkedCount}}</i> items
                </h4>
                <h5>
                  Total <i>{{totalCount}}</i> items
                </h5>
              </div>
              <div class="shipping-total shipping-price">
                <h4 class="">
                  Total Price：<i>{{checkedPrice|currency('$')}}</i>
                </h4>
            ...
            ><a href="javascript:;">Proceed to checkout</a></span>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editCart(itemId, itemCount, checked) {
        this.$store.dispatch('editCartAct', {itemId, itemCount, checked})
      },
      hideCartHandle(flag) {
        this.$store.commit('hideCart', flag)
      },
      checkAllItemsActHandle(allChecked) {
        this.$store.dispatch('checkAllItemsAct', allChecked);
      },
      delAllCheckedItemsActHandle() {
