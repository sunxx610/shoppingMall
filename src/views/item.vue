        ...
        <div class="gallery-wrapper">
          <div class="gallery">
            <div class="thumbnail">
              <ul>
                <li
                    :class="{'on':thumbIndex==index}"
                    v-for="(thumb,index) in itemData.ali_images"
                    @click="selThumbNail(index)"
                ><img
                    :src="'/static/goodsPic/'+thumb"
                ></li>
              </ul>
            </div>
            <div class="thumb">
              <ul>
                <li :class="{'on':thumbIndex==index}" v-for="(thumb,index) in itemData.ali_images"><img
                    :src="'/static/goodsPic/'+thumb"
        ...
        <div class="banner">
          <div class="sku-custom-title">
            <div class="params-price">
              <span><em>$</em><i>{{Number(itemData.price).toFixed(2)}}</i></span>
            </div>
          ...
          <div class="sku-dynamic-params-panel">
            <div class="sku-dynamic-params clear">
              <span class="params-name">Color</span>
              <ul class="params-colors">
                <li :class="{'cur':itemId==color.sku_id}" v-for="color in goodsData.sku_list">
                  <router-link
                      :to="{'name':'Item',query: {itemId:color.sku_id,id}}"
                      :title="color.spec_json.show_name"
                  >
                    <img :src="'/static/goodsPic/'+color.spec_json.image">
                  </router-link>
                </li>
              </ul>
            </div>
            <div class="sku-dynamic-params clear">
              <div class="params-name">Amount</div>
          ...
          <div class="sku-status">
            <div class="cart-operation-wrapper clearfix">
              ...
              <!--<span class="gray-title-btn"><a href="javascript:;">Buy now</a></span>-->
            </div>
          </div>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addCart() {
        let itemId = this.itemId, addCount = this.count;
      ...
      getGoodsList() {
        this.axios.get('/goods', {
          ...
          if (res.status === '0') {
            this.goodsData = res.result.list[0];
            this.itemData = this.goodsData.sku_list.filter(sku => {
              return sku.sku_id === this.itemId;
            })[0]
          }
          else {
          }
